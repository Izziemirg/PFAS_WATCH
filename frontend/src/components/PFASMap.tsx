...
export default function PFASMap({ zoomToZip, onViewDetails }: PFASMapProps) {
  // All States & Refs inside the ONE function
  ...
  // EFFECT A: Watch for ZIP Search Zoom
  useEffect(() => {
  if (!zoomToZip) return;
  ...
  // EFFECT B: Load data when filters change
  useEffect(() => {
    setLoading(true);
  ...
  // Group systems by risk tier for separate traces
  ...
  const traces = tiers.map((tier) => {
    const group = systems.filter(
      (s) => s.risk_tier === tier && s.latitude && s.longitude
    );
    return {
    ...
    <div>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: "13px" }}>
            {loading
              ? "Loading systems..."
              : `${systems.filter(s => s.latitude && s.longitude).length.toLocaleString()} systems displayed`}
          </p>
          {!loading && !includeAll && (
            <p style={{
            ...
            Showing systems with{" "}
            <span style={{ color: "var(--text-secondary)" }}>
             detected or elevated PFAS levels only.
            </span>
            {" "}Toggle{" "}
            <span style={{ color: "var(--accent)" }}>Show All Systems</span>
            {" "}to include systems with no detection.
         </p>
       )}
        </div>
        ...
        <button
          ...
          style={{
            padding: "8px 16px",
            background: includeAll ? "var(--accent-glow)" : "transparent",
            border: `1px solid ${includeAll ? "var(--accent)" : "var(--border-light)"}`,
            borderRadius: "6px",
            color: includeAll ? "var(--accent)" : "var(--text-secondary)",
            fontSize: "12px",
            fontWeight: "500",
            cursor: "pointer",
            transition: "all 0.2s",
      ...
      <div style={{
        ...
        <Plot
          ...
          onRelayout={(e: any) => {
            const updates: any = {};
            if (e["geo.center.lat"] !== undefined) {
              updates.center = {
                lat: e["geo.center.lat"],
                lon: e["geo.center.lon"],
            };
          }
            if (e["geo.projection.scale"] !== undefined) {
              updates.projection = {
                type: "albers usa",
                scale: e["geo.projection.scale"],
            };
          }
            if (Object.keys(updates).length > 0) {
              geoLayoutRef.current = { ...geoLayoutRef.current, ...updates };
          }
          }}
        />
      </div>
      ...
      {selected && (
        <div style={{
          ...
          padding: "1.5rem",
          background: "var(--bg-card)",
          border: `1px solid ${RISK_TIER_COLORS[selected.risk_tier as RiskTier]}40`,
          borderLeft: `3px solid ${RISK_TIER_COLORS[selected.risk_tier as RiskTier]}`,
          borderRadius: "var(--radius-lg)",
          display: "flex",
          justifyContent: "space-between",
          alignItems: "center",
          ...
          <div>
            <div style={{
              ...
              color: RISK_TIER_COLORS[selected.risk_tier as RiskTier],
              marginBottom: "4px",
              fontWeight: "600",
            }}>
              {selected.risk_tier}
              {selected.is_tribal && " · Tribal System"}
            </div>
            <div style={{
              fontFamily: "var(--font-display)",
              fontSize: "1.2rem",
            ...
            <div style={{
              ...
              {selected.state} ·{" "}
              {selected.pws_size === "L" ? "Large System" : "Small System"} ·{" "}
              ID: {selected.pws_id}
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
            ...
            <button
                onClick={() => onViewDetails(selected.pws_id)}
                    style={{
                        padding: "8px 16px",
